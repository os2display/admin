...
angular.module('ikApp').directive('ikSharedChannel', ['$interval', '$location',
  function($interval, $location) {
    ...
    return {
      ...
      link: function(scope) {
        ...
        scope.$watch('ikChannel', function (val) {
          ...
          if (scope.ikChannel.slides.length <= 0) {
            scope.templateURL = 'apps/ikApp/shared/elements/sharedChannel/empty-channel.html?' + window.config.version;
          }
          else {
            scope.templateURL = 'apps/ikApp/shared/elements/sharedChannel/non-empty-channel.html?' + window.config.version;
      ...
      templateUrl: 'apps/ikApp/shared/elements/sharedChannel/shared-channel-template.html?' + window.config.version
    };
  }
]);
