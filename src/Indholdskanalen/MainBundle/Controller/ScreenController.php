                ...
                if (!isset($channelScreenRegion->id)) {
                    ...
                    if (isset($channelScreenRegion->channel)) {
                        ...
                        if (isset($channelScreenRegion->shared_channel)) {
                            // Get shared channel.
                            $sharedChannel = $sharedChannelRepository->findOneByUniqueId(
                                $channelScreenRegion->shared_channel->unique_id
                            );

                            // Get channel from sharing service.
                            $result = $sharingService->getChannelFromIndex(
                                $channelScreenRegion->shared_channel->unique_id,
                                $channelScreenRegion->shared_channel->index
                            );
    ...
    public function screenGetPostAction()
    {
        ...
        $response_data = array(
            ...
            'template' => json_decode(
                $serializer->serialize($screen->getTemplate(), 'json', SerializationContext::create()
    ...
    public function screensGetAction()
    {
        ...
        $response->setContent(
            $serializer->serialize(
                $screenEntities,
                'json',
                SerializationContext::create()->setGroups(array('api-bulk'))->enableMaxDepthChecks()
            )
        );
