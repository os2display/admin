...
Feature: admin
  ...
  Background:
    ...
    And the following users exist:
      ...
      | user     | user     | ROLE_USER | 1: GROUP_ROLE_ADMIN, 2: GROUP_ROLE_USER, 3: GROUP_ROLE_USER |
  ...
  Scenario: Get channels
    ...
    And the JSON node "[0].slides" should have 0 elements
    And the SQL query "SELECT * FROM ik_grouping WHERE entity_type = 'Os2Display\\CoreBundle\\Entity\\Channel'" should return 0 element
  ...
  Scenario: Get channels
    ...
    And the SQL query "SELECT * FROM ik_grouping WHERE entity_type = 'Os2Display\\CoreBundle\\Entity\\Channel'" should return 2 elements
  ...
  Scenario: Get channels
    ...
    And the SQL query "SELECT * FROM ik_grouping WHERE entity_type = 'Os2Display\\CoreBundle\\Entity\\Channel'" should return 1 element

  Scenario: Remove channel
    When I send a "DELETE" request to "/api/channel/1"
    Then the response status code should be 200
    And the SQL query "SELECT * FROM ik_grouping WHERE entity_type = 'Os2Display\\CoreBundle\\Entity\\Channel'" should return 0 elements
