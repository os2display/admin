...
(function () {
   ...
   */
  app.directive('sharedChannelPickerWidget', ['sharedChannelFactory', 'busService',
    function (sharedChannelFactory, busService) {
      return {
        restrict: 'E',
        replace: true,
        templateUrl: 'bundles/os2displayadmin/apps/ikApp/shared/widgets/sharedChannelPickerWidget/shared-channel-picker-widget.html?' + window.config.version,
        scope: {
          screen: '=',
          region: '='
        },
          ...
          sharedChannelFactory.getSharingIndexes().then(
            ...
            function error(reason) {
              busService.$emit('log.error', {
                'cause': reason,
                'msg': 'Kunne ikke hente delingsindeks.'
              });
            }
          );
