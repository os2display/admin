...
angular.module('ikApp').directive('ikChannel', ['$interval', '$location',
  function($interval, $location) {
    ...
    return {
      ...
      link: function(scope, element, attrs) {
        ...
        attrs.$observe('ikChannel', function(val) {
          ...
          if (scope.ikChannel.slides.length <= 0) {
            scope.templateURL = 'bundles/os2displayadmin/apps/ikApp/shared/elements/channel/empty.html?' + window.config.version;
          }
          else {
            scope.templateURL = 'bundles/os2displayadmin/apps/ikApp/shared/elements/channel/non-empty.html?' + window.config.version;
      ...
      templateUrl: 'bundles/os2displayadmin/apps/ikApp/shared/elements/channel/channel-template.html?' + window.config.version
    };
  }
]);
